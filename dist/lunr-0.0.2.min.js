...
//
var Lunr=function(c,f){var b=new Lunr.Index(c);f.call(b,b);return b};Lunr.utils={uniq:function(c){return c.reduce(function(c,b){c.indexOf(b)===-1&&c.push(b);return c},[])},intersect:function(c){var f=[].slice.call(arguments,1);return this.uniq(c).filter(function(b){return f.every(function(a){return a.indexOf(b)>=0})})}};Array.prototype.detect=function(c,f){for(var b=this.length,a=null,g=0;g<b;g++)if(c.call(f,this[g],g,this)){a=this[g];break}return a};
Lunr.Trie=function(){var c=function(b){this.children=[];this.key=b;this.value=[]};c.prototype={childForKey:function(b){var a=this.children.detect(function(a){return a.match(b)});a||(a=new c(b),this.children.push(a));return a},setValue:function(b){this.value.push(b)},match:function(b){return b===this.key}};var f=function(){this.root=new c("")};f.prototype={get:function(b){var a=this;return this.keys(b).reduce(function(b,d){a.getNode(d).value.forEach(function(a){b.push(a)});return b},[])},getNode:function(b){var a=
function(b,d){if(!d.length)return b;return a(b.childForKey(d.charAt(0)),d.slice(1))};return a(this.root,b)},keys:function(b){var a=[];b=b||"";var c=function(b,e){b.value.length&&a.push(e);b.children.forEach(function(a){c(a,e+a.key)})};c(this.getNode(b),b);return a},set:function(b,a){var c=function(b,e){if(!e.length)return b.setValue(a);c(b.childForKey(e.charAt(0)),e.slice(1))};return c(this.root,b)}};return f}();Lunr.Index=function(c){this.name=c;this.refName="id";this.fields={};this.trie=new Lunr.Trie};
Lunr.Index.prototype={add:function(c){(new Lunr.Document(c,this.refName,this.fields)).words().forEach(function(c){this.trie.set(c.id,c.docs[0])},this)},field:function(c,f){this.fields[c]=f||{multiplier:1}},ref:function(c){this.refName=c},search:function(c){c=c.split(" ").map(function(c){c=new Lunr.Word(c);if(!c.isStopWord())return c.toString()}).filter(function(c){return c}).map(function(c){return this.trie.get(c).sort(function(b,a){if(b.score<a.score)return 1;if(b.score>a.score)return-1;return 0}).map(function(b){return b.documentId})},
this);return Lunr.utils.intersect.apply(Lunr.utils,c)}};Lunr.Document=function(c,f,b){this.original=c;this.fields=b;this.ref=c[f]};
