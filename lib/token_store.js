...
lunr.TokenStore.prototype.add = function (token, doc) {
  this.forwards.add(token)
  this.backwards.add(lunr.utils.reverseString(token))
...
lunr.TokenStore.prototype.expand = function (token) {
  ...
  var parts = token.split("*"),
      suffix = lunr.utils.reverseString(parts[1] || ""),
      prefix = parts[0]

  if (suffix && prefix) {
    var forwardSet = this.forwards.suffixes(prefix)

    var backwardSet = this.backwards.suffixes(suffix)
    ...
    return forwardSet.intersect(backwardSet).toArray()

  } else if (suffix) {
    return this.backwards.suffixes(suffix)
      .map(lunr.utils.reverseString)

  } else if (prefix) {
    return this.forwards.suffixes(prefix).toArray()
  }
}
