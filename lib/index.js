...
lunr.Index.prototype.add = function (doc) {
  ...
  for (var i = 0; i < allDocumentTokens.length; i++) {
    ...
    this.tokenStore.push(token, { ref: docRef, tf: tf })
  };
}
...
lunr.Index.prototype.remove = function (doc) {
  ...
  docTokens.forEach(function (token) {
    this.tokenStore.reject(token, docRef)
  }, this)
}
...
lunr.Index.prototype.search = function (query) {
  ...
  queryTokens
    ...
    .forEach(function (token, i, tokens) {
      ...
      Object.keys(this.tokenStore.get(token)).forEach(function (ref) { documentSet.add(ref) })
    }, this)
  ...
  var documentVectors = documentSet.map(function (documentRef) {
    ...
    for (var i = 0; i < documentTokensLength; i++) {
      var token = documentTokens.elements[i],
          tf = this.tokenStore.getNested(token, documentRef).tf,
          idf = this.idf(token)
