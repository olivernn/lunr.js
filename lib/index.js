...
lunr.Index.prototype.search = function (queryString) {
  ...
  var postings = queryTokens.map(function (token) {
    var nodes = lunr.utils.arrayWrap(this.store.get(token, this.store.root))
    return lunr.utils.flatten(nodes.map(function (node) { return node._values }))
  }, this)
  ...
  return Object.keys(vectors).sort(function(a, b) {
    var aSimilarity = vectors[a].similarity(queryVector),
        bSimilarity = vectors[b].similarity(queryVector)
